...
from models.prediction.svm_predictor import SVMPredictor
...
def svm_predict(production_line_code, fault_code, random_state=42):
    """
    Train and evaluate SVM BASELINE model using raw data only.
    
    Baseline processing pipeline:
    1. Load raw data (NO temporal features)
    2. Split train/test (NO CBSS balancing)
    3. Train SVM directly (NO RF feature selection)
    ...
    # Initialize SVM baseline predictor with integrated progress display
    predictor = SVMPredictor(random_state=random_state, show_progress=True)
    ...
    model_path = os.path.join(MODEL_DIR, f'svm_production_line_{production_line_code}_fault_{fault_code}.pkl')
    predictor.evaluate_and_save(x_test, y_test, model_path)
    
    print(f"Baseline SVM model saved to: {model_path}")

if __name__ == "__main__":
    
    parser = argparse.ArgumentParser(description='Train SVM baseline model for comparison with hybrid RF-XGBoost method')
