#!/usr/bin/env python3
"""
Script to train 20 XGBoost models with different random seeds for journal requirements.
This ensures reproducibility and statistical significance of results.
"""
...
def run_xgboost_experiments(production_line=1, fault_code=1001):
    """
    Run 20 XGBoost training experiments with different random seeds.
    Includes seed 42 as requested.
    """
    ...
    print(f"Starting XGBoost experiments for Production Line {production_line}, Fault Code {fault_code}")
    ...
    experiment_log = f"experiments/xgboost_batch_{production_line}_{fault_code}_{timestamp}.log"
    
    successful_runs = 0
    failed_runs = 0
    
    with open(experiment_log, 'w') as log_file:
        log_file.write(f"XGBoost Batch Experiment Log\n")
        ...
        for i, seed in enumerate(random_seeds, 1):
            print(f"Run {i}/20: Training with random seed {seed}...")
            
            # Construct the command - force new feature selection each time
            cmd = [
                'uv', 'run', 'python', 'scripts/train_xgboost.py',
                '--production_line', str(production_line),
                '--fault_code', str(fault_code),
                '--random-state', str(seed),
                '--parameter-opt'  # Enable parameter optimization for more randomness
            ]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Run multiple XGBoost training experiments with different random seeds'
    )
    parser.add_argument(
        '--production_line', 
        type=int, 
    ...
    run_xgboost_experiments(args.production_line, args.fault_code)