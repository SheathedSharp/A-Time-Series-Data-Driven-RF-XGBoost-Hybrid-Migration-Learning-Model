...
def lightgbm_predict(production_line_code, fault_code, random_state=42):
    """
    Train and evaluate LightGBM BASELINE model using raw data only.
    
    Baseline processing pipeline:
    1. Load raw data (NO temporal features)
    2. Split train/test (NO CBSS balancing)
    3. Train LightGBM directly (NO RF feature selection)
    ...
    print(f"Baseline LightGBM model saved to: {model_path}")

if __name__ == "__main__":
    
    parser = argparse.ArgumentParser(description='Train LightGBM baseline model for comparison with hybrid RF-XGBoost method')
