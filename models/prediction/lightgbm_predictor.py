...
class LightGBMBaselinePredictor:
    ...
    def __init__(self, random_state=42, show_progress=True):
        """Initialize LightGBM baseline predictor with default parameters and fixed random state."""
    ...
    def train(self, x_train, x_test, y_train, y_test):
        """Train the LightGBM BASELINE model with integrated progress display.
    ...
    def _train_with_progress(self, x_train, x_test, y_train, y_test, status):
        ...
        status.update("Initializing LightGBM BASELINE training...")
        ...
        # Calculate class_weight to handle class imbalance in baseline
        neg_count = (y_train == 0).sum()
        pos_count = (y_train == 1).sum()
        scale_pos_weight = neg_count / pos_count if pos_count > 0 else 1
        
        status.update("Training LightGBM baseline model...")    
        ...
        status.complete("LightGBM BASELINE model training completed successfully")
        return self.model
    ...
    def _train_without_progress(self, x_train, x_test, y_train, y_test):
        ...
        # Calculate class_weight to further handle class imbalance
        neg_count = (y_train == 0).sum()
        pos_count = (y_train == 1).sum()
        scale_pos_weight = neg_count / pos_count if pos_count > 0 else 1
