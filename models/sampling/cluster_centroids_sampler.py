...
class ClusterCentroidsSampler:
    """Cluster Centroids Under Sampler wrapper.
    
    This sampler uses ClusterCentroids algorithm which applies K-means clustering
    to the majority class and replaces samples with cluster centroids. This is
    a more sophisticated approach than random sampling as it preserves the 
    underlying data distribution structure.
    """
    ...
    def __init__(self, 
                 ...
                 random_state: int = 42,
                 voting: str = 'auto'):
        """Initialize the Cluster Centroids sampler.
        
        Args:
            estimator: Clustering estimator (default: None, uses KMeans)
            sampling_strategy (str): Sampling strategy (default: 'auto')
            random_state (int): Random seed for reproducibility
            voting (str): Voting strategy for centroids (default: 'auto')
        """
    ...
    def balance_dataset(self, x_train: pd.DataFrame, x_test: pd.DataFrame, 
        ...
        """Balance training dataset using Cluster Centroids.
        ...
        with self.progress.sampling_status() as status:
            status.update("Initializing Cluster Centroids sampler...")
            ...
            status.update("Applying Cluster Centroids to training data...")
            
            try:
                # Apply Cluster Centroids to training data
                ...
                status.complete("Cluster Centroids sampling completed")
                ...
                self.progress.display_warning(f"Cluster Centroids failed, falling back to random sampling: {str(e)}")
    ...
    def _display_sampling_statistics(self, x_train_orig, y_train_orig, x_train_bal, y_train_bal):
        """Display Cluster Centroids sampling statistics."""
        removed_samples = len(x_train_orig) - len(x_train_bal)
        stats_data = {
            "Sampling Method": "Cluster Centroids",
            "Sampling Strategy": self.sampling_strategy,
            "Voting Strategy": self.voting,
        ...
        self.progress.display_results_table("Cluster Centroids Sampling Statistics", stats_data)
